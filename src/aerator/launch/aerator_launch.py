from launch import LaunchDescription
from launch_ros.actions import Node
from ament_index_python.packages import get_package_share_directory
import os

def generate_launch_description():
    # Get the path to the YAML config file
    config = os.path.join(
        get_package_share_directory('aerator'),  # 'aerator' is the package name
        'config',
        'aerator_config.yaml'  # This is the config file
    )

    return LaunchDescription([
        Node(
            package='aerator',
            executable='aerator_la_control',
            output='screen',
            name='aerator_la_control',
            parameters=[config]  # Pass the config file to this node
        ),
        Node(
            package='aerator',
            executable='aerator_ultra_sensor',
            output='screen',
            name='aerator_ultra_sensor',
            parameters=[config]  # Pass the config file to other nodes if they need it
        ),
        Node(
            package='aerator',
            executable='aerator_planner',
            output='screen',
            name='aerator_planner',
            parameters=[config]
        )
    ])
